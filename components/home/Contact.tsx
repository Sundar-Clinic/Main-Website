  ...
  async function onSubmit(values: z.infer<typeof contactFormSchema>) {
    try {
      ...
      if (!executeRecaptcha) {
        toast({
          title: "Error",
          description: "Recaptcha is not loaded, try again later.",
          variant: "destructive",
        });
        return;
      }
      ...
      const response = await axios.post<{ message: string }>("/api/contact", {
        ...values,
        token,
      });
      if (
      ...
      if (error instanceof AxiosError) {
        switch (error.response?.data?.message) {
          ...
          case "contact/failed-reCAPTCHA-validation": {
            toast({
              title: "Error",
              description: "Failed reCAPTCHA validation, try again.",
              variant: "destructive",
            });
            break;
          }
        ...
        toast({
          title: "Error",
          description: "Unable to submit form at the moment, try again later.",
          variant: "destructive",
        });
      }
    } finally {
        ...
        <div className="w-full h-full p-2">
          {alreadySubmitted ? (
            <div>
              ...
              <p className="text-lg text-center">{t("success.subheading")}</p>
            </div>
          ) : (
            <Form {...form}>
              <form
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="8939881708" type="tel" {...field} />
                      </FormControl>
                      <FormDescription>
